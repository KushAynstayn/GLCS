...
async function startInsert(override = false) {
    ...
    try {
        ...
        if (data.duplicate) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    showOverrideLoading(); // 🔥 show loading
                    startInsert(true);
                }
            });
        ...
        Swal.fire({
            ...
            text: `${data.inserted} rows inserted successfully`
        });
...
function showOverrideLoading() {
    Swal.fire({
        title: 'Overriding Data...',
        text: 'Please wait while the system replaces existing records.',
